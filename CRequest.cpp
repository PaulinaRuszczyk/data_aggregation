...
int CRequest::retriveData(const std::string& identNumber) {
    try {
        ...
        std::string request = "curl --request GET \\\n"
                              "  --url 'https://api.kingfisher.com/v1/mobile/stores/CAPL?filter%5Bean%5D=" +
                              identNumber +
                              "&include=clickAndCollect%2Cstock&nearLatLong=52.172491%2C20.938261&page%5Bsize%5D=250' \\\n"
